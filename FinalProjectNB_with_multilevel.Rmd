...
---
title: "Final Project"
author: "Sean Burke, Allan Julian, Jared Hayes" 
output:
...
studentDS <- read.csv("StudentSucessDataset.csv")

```

## Introduction (Motivation)

- In today's dynamic educational landscape, it's crucial to delve deeper into what influences student graduation and employment outcomes. Our presentation aims to unravel the complexities surrounding this topic by examining various factors that shape students' paths to success. From demographic data to socioeconomic factors and academic performance indicators, we'll explore the intricate web of influences that impact student employment prospects. 

- The data set we are studying contains 4424 observations, in which each case is a university student. In addition, there are 35 variables that contain a variety of information regarding family, socioeconomics, semester information, and other demographical information. 

- Our response variable is `Target`, which describes the status of the students. 

- Using the other variables, our goal is to look at the qualities of some of the variables and create models predicting our response.

- Our dataset is cited as: [Valentim Realinho, Jorge Machado, Luís Baptista, & Mónica V. Martins. (2021). Predict students' dropout and academic success (1.0) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.5777340](https://zenodo.org/records/5777340)

## Mutate Data

- Using information about the dataset, we mutated the dataset to include specific information for each categorical variable (excluding the Target).
...
studentDS %>%
  ...
  mutate(freq = n, total = sum(freq), prop = freq / total) %>%
  ungroup() -> plot_data

# Plot using ggplot2
ggplot(plot_data, aes(x = "", y = prop, fill = Target)) +
  geom_bar(stat = "identity", width = 1, color = "black") +
  coord_polar(theta = "y") +
  facet_wrap(~ International) +
  scale_fill_manual(values = c(psuPalette[2], boastPalette[3], boastPalette[1])) +
  theme_void() +
  theme(legend.position = "none") +
  labs(
    title = "Pie Chart of Student Status by International Status",
    x = NULL,
    y = NULL
  )
...
- Here we see a very slightly higher proportion of Non-International graduates.

```{r}
#View Proportion of Student Status
studentDS1 %>%
  count(Nationality, Target) %>%
...
ggplot(plot_data, aes(x = "", y = prop, fill = Target)) +
  ...
  theme(legend.position = "none") +
  labs(
    title = "Pie Chart of Student Status by Nationality",
    x = NULL,
    y = NULL
  )
...
- From the Grid Pie plot, it is easy to see that different nationalities have different status proportions. We will explore this differences in future sections. 

- `NOTE:` There are many instances where certain nationality groups contain very small sample sizes as low as 1.

```{r}
#Calculate means
meanGrade1 <- studentDS %>% 
  group_by(Target) %>%
  summarize(
    N = n(),
...
ggplot(
  ...
  geom_point(
    data = meanGrade1,
    mapping = aes(x = Target, y = avgGrade, color = "Mean of Grade"),
    shape = 6,
    size = 2
  ) +
  scale_color_manual(values = psuPalette[2], name = "", labels = "Mean of Grade") + 
...
ggplot(
  data = studentDS,
  mapping = aes(x = Age.at.enrollment)
  ) +
    geom_histogram(
      color = "black",
      fill = boastPalette[4],
  ...
  labs(
    title = "Histogram of Age at Enrollment",
    x = "Age at Enrollment",
    y = "Frequency"
  )
```
...
- The median age is `20` years old with
Some very extreme value at the right (`70`).
...
ggplot(plot_data, aes(x = "", y = prop, fill = Target)) +
  ...
  theme(legend.position = "none") +
  labs(
    title = "Pie Chart of Student Status by Debt Status",
    x = NULL,
    y = NULL
  )
...
`NOTE:` `1` indicates the student is in debt and `0` indicates the student is not in debt.

**Comments:**

- Intuitively, we see students with more debt tend to exhibit a higher proportion of drop-out.


```{r}
#View Proportion of Student Status
studentDS %>%
  count(Scholarship.holder, Target) %>%
...
ggplot(plot_data, aes(x = "", y = prop, fill = Target)) +
  ...
  theme(legend.position = "none") +
  labs(
    title = "Pie Chart of Student Status by Scholarship Status",
    x = NULL,
    y = NULL
  )
...
`NOTE:` `1` indicates the student has some sort of scholarship and `0` indicates the student is not a scholarship holder.

**Comments:**

- We see scholarship holders have a higher proportion of Graduates, which is expected.
...
- For our Generalized Linear Model (GLM), we will be creating a Multinomial Logistic Regression Model due to the 3 levels of our response.

- First we will perform a variable selection to find the best predictors for our model. Then, we will build our model using the variables selected. After, we will assess our model using overall accuracy as our indicator.
...
- We will perform a backwards feature selection based on AIC criterion, because we are interested in making predictions.

- We are interested in minimizing our model complexity. And, this is method is automatic.

```{r}
#Build full and base models
...
`NOTE:` Given the Nuances in the Data (as seen in the correlation heatmap), it is a good practice to account for interaction terms when building the model.

- Our Initial model before the feature selection included `34` Main Effects and `33` Interactions.

- After performing the backwards selection we ended up with `25` Main Effects and `13` Interactions.

### Modeling

**Initial Comments: **

- Using the selected predictors we will create a multinomial model using the `multinom` function.
...
- If we look back at our EDA, our `International` variable seemed to have little effect.

- While we don't know for sure without sufficient data, there is a high possibility of confounding variables or other factors that influenced this.

- *Interpretations:* The odds that a student is enrolled vs dropped out, after controlling for the other variables change by a factor of `10.5276` as we move from non-international to international status. Likewise, the odds that a student is graduated vs dropped out, after controlling for the other variables change by a factor of `35.83769` as we move from non-international to international status.
...
- Looking at the confusion matrix, we obtain an overall accuracy of 77.8481%

## Additional Model 

- To further enhance our predictions, we are interested in implementing other machine learning techniques

- Because of the high volume of categorical data and interactions, we were interested in seeing if using decision trees would be beneficial.

- Specifically we will be using a bagging (bootstrap aggregated) technique call random foresting. 
...
- For this model, we will use only the main effects selected in our multinomial model, since random forest decision trees handle interactions well. 

### Modeling/Assessment

```{r}
#Create Random Forest Object
rf.object <- randomForest(as.factor(Target) ~ Application.order + Course + Nacionality + Mother.s.qualification + 
    Father.s.qualification + Mother.s.occupation + Father.s.occupation + 
    Displaced + Debtor + Tuition.fees.up.to.date + Gender + Scholarship.holder + 
    Age.at.enrollment + International + Curricular.units.1st.sem..credited. + 
    Curricular.units.1st.sem..enrolled. + Curricular.units.1st.sem..approved. + 
...
```{r}
#Display Information including Confusion matrix and Importance
rf.object

rf.object$importance
...
- Looking at the confusion matrix, we obtain an overall accuracy of 78.37% which is higher than the accuracy obtained in the regular multinomial regression model. 

- Using MeanDecreaseGini, our top 3 important variables are as follows: The `Curricular.units.2nd.sem..approved.`, the `Curricular.units.2nd.sem..grade.`, and the `Curricular.units.1st.sem..evaluations.`

- Although there seems to be a difference in what was chosen as the most important variable in the multinomial logistic regression model in comparison to the random forest model, it should be noted that these top 3 important variables selected here were still effective predictors in the multinomial logistic regression model. 

## MultiLevel Modeling Section

**Variable of Interest: ** `Unemployment.rate` 

-  Unemployment rates with the model will allow us to pinpoint demographic and socioeconomic factors.

- Analyzing unemployment rates using a robust statistical model fosters institutional accountability.

- The relationship between unemployment rates and academic performance indicators will help provide insights into the factors driving student success (our target variable).

- Insights gained from analyzing unemployment rates can hopefully be used to inform the development of effective policies and programs aimed at reducing student unemployment.

### EDA (Multilevel)

```{r}
#View unemployment rate
ggplot(
  data = studentDS,
  mapping = aes(x = Unemployment.rate)
) +
  geom_histogram(
    color = "black",
    fill = boastPalette[2],
...
- Here we see a slightly right-skewed, with the data being most frequent around 12% 

```{r}
#View Unemployment rate by Course
ggplot(
  data = studentDS1,
  mapping = aes(x = Unemployment.rate)
) +
  ...
  facet_wrap(Course~.) +
  labs(
    title = "Histogram of Unemployment Rate by Course",
   x = "Unemployment Rate",
   y = "Frequency"
  )
```
...
- Here, we can see the distribution of the Unemployment rate by each Course.

- In particular, we see majors like Nursing and Management distributed more towards lower unemployment rates, which is expected.

- Along the same lines, we see courses like Agronomy and Communication Design distributed more towards the higher unemployment rates. 

- There seems to be a trend where more essential jobs like health are reporting lower unemployement rates, which is to be expected.

### MultiLevel Model

```{r}
#Create MultiLevel Model
Umodel <- lmer(Unemployment.rate ~ Marital.status + Nationality + Application.mode + Previous.qualification + Mother.s.qualification + Father.s.qualification + Mother.s.occupation + Father.s.occupation + (1 | Course), data = studentDS1)
```
...
- **Model Complexity**: The model includes a relatively large number of predictor variables, encompassing factors such as marital status, nationality, previous qualification, and parental qualifications and occupations. This complexity can potentially lead to overfitting and difficulties in model interpretation.

- **Random Effects**: Random intercepts are included for the effect of `Course`, indicating that there is variability in the intercepts across different courses. This acknowledges the nested structure of the data, where students are clustered within courses. The estimated variance of the random intercepts is 0.3195, suggesting that there is variability in the intercepts among courses.

- **Model Fit**: The REML criterion at convergence is 19851.4, providing a measure of the model's goodness of fit. Lower values of the REML criterion indicate better fit, although the absolute value itself does not offer a straightforward interpretation. However, it can be used for model comparison, where models with lower REML values are preferred if they provide a similar level of explanatory power.


We've found that the unemployment rates can vary across different courses. This means that even if two students have similar backgrounds and characteristics, they might still have different chances of being unemployed depending on the course they're enrolled in. It's like saying that the environment or atmosphere of the course they're taking can also play a role in their employment outcomes. Overall, our analysis highlights the importance of considering various factors beyond just academic performance when understanding students' employment outcomes, and it underscores the need for tailored support and interventions to ensure every student has the opportunity to thrive in their education and future career.

## Conclusion

- In conclusion, our project has provided valuable insights into the complex interplay of factors influencing student employment outcomes in higher education. Through the application of sophisticated statistical modeling techniques, we have elucidated the multifaceted nature of student success, considering demographic, socioeconomic, and academic performance indicators.
