...
corMat <- cor(studentDS %>% select(!Target & !sex), use = "complete.obs")
...
fullModel <- multinom(Target ~ .-Target - sex, data = studentDS)
...
bsModel <- multinom(Target ~ Application.mode + Application.order + Course + Previous.qualification + 
    Nacionality + Mother.s.qualification + Mother.s.occupation + 
    ...
    Curricular.units.2nd.sem..without.evaluations. + Unemployment.rate, data = studentDS)

#Summary Table
tidy(bsModel, conf.int = TRUE, exponentiate = FALSE) %>%
...
$$\log\left(\frac{\hat{p}_{i, \text{Enrolled}}}{\hat{p}_{i, \text{Dropout}}}\right) = 0.1247949 + 0.009149317 x_{\text{Application.mode}} - 0.1084979 x_{\text{Application.order}} - 0.05984535 x_{\text{Course}} - 0.007143544 x_{\text{Previous.qualification}} - 0.1607959 x_{\text{Nacionality}} - 0.028105054 x_{\text{Mother.s.qualification}} + 0.08921911 x_{\text{Mother.s.occupation}} - 0.3389803 x_{\text{Displaced}} - 0.2269397 x_{\text{Debtor}} + 1.981863 x_{\text{Tuition.fees.up.to.date}} - 0.2478873 x_{\text{Gender}} + 0.07732892 x_{\text{Scholarship.holder}} - 0.04821212 x_{\text{Age.at.enrollment}} + 2.26295 x_{\text{International}} - 0.06734891 x_{\text{Curricular.units.1st.sem..credited.}} + 0.03514532 x_{\text{Curricular.units.1st.sem..evaluations.}} + 0.118055 x_{\text{Curricular.units.1st.sem..approved.}} - 0.04355277 x_{\text{Curricular.units.1st.sem..grade.}} - 0.22199676 x_{\text{Curricular.units.2nd.sem..credited.}} - 0.3572505 x_{\text{Curricular.units.2nd.sem..enrolled.}} + 0.09406026 x_{\text{Curricular.units.2nd.sem..evaluations.}} + 0.3631424 x_{\text{Curricular.units.2nd.sem..approved.}} + 0.07983063 x_{\text{Curricular.units.2nd.sem..grade.}} + 0.0943232 x_{\text{Curricular.units.2nd.sem..without.evaluations.}} - 0.10141801 x_{\text{Unemployment.rate}} + \epsilon$$

$$\log\left(\frac{\hat{p}_{i, \text{Graduate}}}{\hat{p}_{i, \text{Dropout}}}\right) = -0.3130005 - 0.015365970 x_{\text{Application.mode}} - 0.0764883 x_{\text{Application.order}} - 0.10971874 x_{\text{Course}} + 0.027356904 x_{\text{Previous.qualification}} - 0.2458373 x_{\text{Nacionality}} - 0.008394626 x_{\text{Mother.s.qualification}} + 0.06949083 x_{\text{Mother.s.occupation}} - 0.3178092 x_{\text{Displaced}} - 1.0343931 x_{\text{Debtor}} + 3.069092 x_{\text{Tuition.fees.up.to.date}} - 0.5051549 x_{\text{Gender}} + 0.73427631 x_{\text{Scholarship.holder}} - 0.04392779 x_{\text{Age.at.enrollment}} + 3.382372 x_{\text{International}} - 0.28985793 x_{\text{Curricular.units.1st.sem..credited}} - 0.04487982 x_{\text{Curricular.units.1st.sem..evaluations}} + 0.6676107 x_{\text{Curricular.units.1st.sem..approved}} - 0.10231861 x_{\text{Curricular.units.1st.sem..grade}} - 0.08366331 x_{\text{Curricular.units.2nd.sem..credited}} - 1.1051600 x_{\text{Curricular.units.2nd.sem..enrolled}} - 0.01985748 x_{\text{Curricular.units.2nd.sem..evaluations}} + 1.1129269 x_{\text{Curricular.units.2nd.sem..approved}} + 0.14357216 x_{\text{Curricular.units.2nd.sem..grade}} + 0.2142498 x_{\text{Curricular.units.2nd.sem..without.evaluations}} - 0.08581726 x_{\text{Unemployment.rate}} + \epsilon$$

**Comments: **

- Looking at the summary table the most effective predictor present in the model is `International` with a p-value of `0.027` and  `0.005`, as well as a coefficient with magnitude of `2.263` and `3.382` when considering Enrolled vs. Dropout and Graduate and Graduate vs. Dropout respectively.

- The 95% confidence interval for the impact of `International` on the odds that a student is Enrolled vs Dropped out `(1.287883, 71.73652)`.

- *Interpretation: * We are 95% confident that the odds that a student is Enrolled vs Dropped out change by a factor between  `1.287883` and `71.73652` as we move from non-international to international.
